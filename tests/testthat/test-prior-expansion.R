...
ts <- simulate_model(model, individual_priors, sequence_length = 1)
...
test_that("Nonsensical priors are correctly caught", {
  ...
  expect_error(simulate_model(model, parameters = "asdf"), error_msg)
  expect_error(simulate_model(model, parameters = list(123)), error_msg)
  expect_error(simulate_model(model, parameters = list(123, xyz = 42)), error_msg)
})
...
test_that("With the same seed, both sets of priors give the same tree sequence", {
  ...
  set.seed(42)
  ts1 <- simulate_model(model, individual_priors, sequence_length = 1, engine_args = list(random_seed = 42), mutation_rate = 1e-8)

  set.seed(42)
  ts2 <- simulate_model(model, expanded_priors, sequence_length = 1, engine_args = list(random_seed = 42), mutation_rate = 1e-8)

  set.seed(42)
  ts3 <- simulate_model(model, templated_priors, sequence_length = 1, engine_args = list(random_seed = 42), mutation_rate = 1e-8)
...
test_that("expand_formulas() catches multiple template matches", {
  ...
  expect_error(
    simulate_model(model, templated_priors),
...
test_that("Catch attempts at templating of vectorized priors", {
  ...
  expect_error(quiet(simulate_model(model, list(broken_prior))), error_msg)
})
...
test_that("catch attempts at templating of vectorized priors", {
  ...
  expect_error(simulate_model(model, broken_priors),
