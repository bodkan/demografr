...
**Also, this is something the user never has to do or care about**, it is done automatically and internally when they call:

```{r, eval=FALSE}
data <- simulate_abc(model, <priors>, functions, observed, ...)
```

Only then, during each ABC replicate simulation, _demografr_ internally performs the equivalent of this:

```{r, eval = FALSE}
# iterate over the list of priors, sample from each and use the sampled
# values for simulating a model
...
# iterate over the list of priors, sample from each and use the sampled
# values for simulating a model
lapply(priors, sample_prior) %>% lobstr::tree()
```
