...
[_msprime_](https://tskit.dev/msprime/docs/stable/intro.html)? This vignette
explains how you can use any standard SLiM or _msprime_ script as a simulation
engine in a standard _demografr_ pipeline without _slendr_.
...
## ABC pipeline using custom-defined simulation scripts
...
We want to infer the posterior distribution of $N_e$ but not via the normal
_slendr_ interface (as shown [here](vignette-01-basics.html)) but using a
simple SLiM or Python script.
...
functions <- list(
  diversity = function(ts) {
    ts_diversity(ts, list(pop = seq(0, 99))) # compute using 100 chromosomes
  }
)
...
library(future)
plan(multisession, workers = availableCores()) # parallelize across all 96 CPUs

data <- simulate_abc(python_script, priors, functions, observed, iterations = 5000,
