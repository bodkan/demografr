...
python \
  ...
  --path <path to the directory with simulation results> \
...
```
slim \
  -d "path='<path to the directory with simulation results>'" \
  <... your model parameters ...>
```
...
```{r}
result_msprime <- simulate_model(model = python_script, parameters = list(Ne = 123), format = "files", data = data)
```
