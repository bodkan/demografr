...
ts_m <- simulate_ts(py_script, parameters = list(Ne = 123), mutation_rate = 1e-8)
```
...
The functions works as expected&mdash;we have a single population and want to compute nucleotide diversity in the whole population, and this is exactly what we get.
...
Looking good! Now let's first run ABC simulations. Note the use of the script where we would normally plug in a slendr model function in place of the `model` argument:
...
data <- simulate_abc(
  model = py_script, priors, functions, observed, iterations = 10,
