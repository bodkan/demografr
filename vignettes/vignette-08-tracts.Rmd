...
gf <- gene_flow(from = nea, to = eur, proportion = 0.03, start = 55000, end = 55000 - 30)
...
```{r}
model <- function(admixture_time, admixture_prop) {
  generation_time <- 30
  ...
  gf <- gene_flow(
    from = nea, to = eur,
    rate = admixture_prop,    # first model parameter
...
gf <- gene_flow(from = nea, to = eur, proportion = 0.03, start = 55000, end = 54000)
...
model <- function(admixture_time, admixture_prop) {
  ...
  gf <- gene_flow(from = nea, to = eur, proportion = admixture_prop,
...
```{r}
truth_df <- data.frame(
  param = c("admixture_prop", "admixture_time"),
  value = c(0.03, 55000)
)
