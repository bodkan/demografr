...
```{r}
# bench_df <- lapply(c(1, 10, 100, 1000, 10000, 100000) * 1e3, function(sequence_length) {
x <- Sys.time()
bench_df <- lapply(c(1, seq(1000, 100000, by = 1000)) * 1e3, function(sequence_length) {
  ...
  ts_time <- system.time(ts_result <- ts_run(ts))
...
bench_df %>%
  ...
  ggplot(aes(alpha_disk, alpha_ts, color = sequence_length  / 1e6)) +
  geom_point() +
