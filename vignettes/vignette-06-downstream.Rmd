...
```{r}
asd <- c(rep("modelX", 1000), rep("modelY", 1000), rep("modelZ", 1000))
stat <- rbind(
  dataX$simulated,
  dataY$simulated,
  dataZ$simulated
...
cv.modsel <- cv4postpr(asd, stat, nval = 5, tol = 0.01, method = "rejection")
summary(cv.modsel)

plot(cv.modsel, names.arg=c("modelX", "modelY", "modelZ"))
```
