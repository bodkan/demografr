...
```{r, echo=FALSE}
rds_file <- system.file("extdata/vignette_01.rds", package = "demografr")
```
...
observed_stats <- list(
  pi = ts_diversity(ts, sample_sets = samples) %>% mutate(stat = paste0("pi_", set), value = diversity) %>% dplyr::select(stat, value)
)
```
