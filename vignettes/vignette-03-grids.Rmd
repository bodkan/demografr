...
knitr::opts_chunk$set(
  ...
  fig.height = 5,
  dpi = 60,
  eval = slendr_present && file.exists(data_path)
)
```
...
model <- function(rate_ea, rate_aa) {
  ...
  gf <- list(
    ...
    gene_flow(from = eur, to = afr1, start = 5e3, end = 0, proportion = rate_ea),
    gene_flow(from = eur, to = afr2, start = 5e3, end = 0, proportion = rate_ea),

    # gene flow within Africa -- another parameter of interest
    gene_flow(from = afr1, to = afr2, start = 50e3, end = 0, proportion = rate_aa),
    gene_flow(from = afr2, to = afr1, start = 50e3, end = 0, proportion = rate_aa),

    # Neanderthal introgression (fixed to 3%)
    gene_flow(from = nea, to = eur, start = 60e3, end = 50e3, proportion = 0.03)
  )
...
```{r, eval=!file.exists(data_path) && rerun}
