...
This looks quite neat and tidy, but how can we make sure that our tree-sequence summary functions really work? We need a tree-sequence object as an input first! _demografr_ provides an easy way to get an arbitrary tree sequence from a model function via its helper function `simulate_output`. The simples way to use it is like this:
...
ts <- simulate_output(model, parameters = list(rate_ea = 0.1, rate_aa = 0.15), mutation_rate = 1e-8)
```
