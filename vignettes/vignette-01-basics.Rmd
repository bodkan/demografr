...
```{r, echo=FALSE}
data_file <- system.file("examples/01_abc_data.rds", package = "demografr")
```
...
Having verified that all model components are set up correctly, we can proceed to the ABC simulations themselves, using _demografr_'s function `simulate_abc()`:
 ...
 <!-- #eval=!file.exists("inst/extdata/abc_data.rds")} -->
...
```{r, eval=file.exists(data_file)}
data <- readRDS(system.file("examples/01_abc_data.rds", package = "demografr"))
```
