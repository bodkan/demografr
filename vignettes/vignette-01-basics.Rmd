...
Of course, having to run ABC simulations in order to check that the summary functions have been correctly defined would be slow and potentially costly in terms of wasted computation. To speed this process up, _demografr_ provides a helper function `simulate_output()` which allows you to simulate a single tree sequence object from the specified model. You can then use that tree sequence object to develop (and test) your summary functions before proceeding further, like this:

```{r}
ts <- simulate_output(model, priors, sequence_length = 1e6, recombination_rate = 1e-8)
```
