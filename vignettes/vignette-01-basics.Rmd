...
data_path <- here::here("inst/examples/abc_data.rds")

devtools::load_all()
```

This vignette contains an expanded version of the basic ABC inference example from the homepage of _demografr_. It walks through the process of setting up a _demografr_ ABC pipeline step by step.
...
```{r, echo=FALSE}
observed_diversity <- read.table(here::here("inst/examples/observed_diversity.tsv"), header = TRUE)

observed_divergence <- read.table(here::here("inst/examples/observed_divergence.tsv"), header = TRUE)

observed_f4  <- read.table(here::here("inst/examples/observed_f4.tsv"), header = TRUE)
```
...
3. The value of $f_4$(popA, popB; popC, popD):
...
```{r, echo=FALSE}
data_file <- here::here("inst/examples/abc_data.rds")
```
...
saveRDS(tdelta, here::here("inst/examples/abc_tdelta.rds"))
ncores <- future::availableCores()
saveRDS(ncores, here::here("inst/examples/abc_ncores.rds"))
```

```{r, echo=FALSE, eval=file.exists(data_path)}
tdelta <- readRDS(here::here("inst/examples/abc_tdelta.rds"))
ncores <- readRDS(here::here("inst/examples/abc_ncores.rds"))
```
