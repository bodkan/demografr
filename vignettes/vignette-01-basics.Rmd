...
In order to do this, we can use _demografr_'s `predict()` method which accepts two mandatory function arguments: the first is the abc object generated by the `perform_abc()` function (and contains, among other things, the information about the inferred posteriors), and the number of draws to take from the multidimensional posterior distribution of the parameters:
...
**For more details on possible downstream validation and troubleshooting analyses, please see [this vignette](vignette-06-downstream.html).**
