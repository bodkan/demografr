...
get_cache <- function(path) {
  if (Sys.getenv("RERUN") == "TRUE" || file.exists(here::here(file.path("inst/", path))))
...
[_slendr_](https://bodkan.net/slendr/) on which _demografr_ relies on for building
...
[tree-sequence functions](https://bodkan.net/slendr/reference/index.html#tree-sequence-statistics)
...
function is defined correctly (meaning that it returns an $f_4$ statistics table
