...
To recap the motivation of the example, imagine that we sequenced genomes of individuals from three populations "p1", "p2", and "p3".
...
```{r}
data <- readRDS(here::here("inst/extdata/data.rds"))
```
...
```{r, demografr_posterior_Ne, fig.width=10, fig.height=7}
plot_posterior(abc, type = "Ne")
```

```{r, demografr_posterior_Tsplit, fig.width=10, fig.height=7}
...
```{r, abc_diagnostic_Ne, fig.width=10, fig.height=7}
