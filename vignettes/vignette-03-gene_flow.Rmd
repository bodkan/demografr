...
```{r, echo=FALSE}
rds_file <- system.file("extdata/vignette_01_gene_flow.rds", package = "demografr")
```
...
p4 <- population("p4", time = 8000, N = 2000, parent = p3)

gf <- list(
  gene_flow(from = p2, to = p4, start = 9200, end = 9201, rate = 0.25),
  gene_flow(from = p2, to = p1, start = 7000, end = 7001, rate = 0.10)
)
...
bench <- microbenchmark::microbenchmark(
  mclapply      = {data <- simulate_abc(model, priors, summary_funs = functions, observed_stats = observed_stats, iterations = 1000, mutation_rate = 1e-8, execution = "mclapply")},
  future_lapply = {data <- simulate_abc(model, priors, summary_funs = functions, observed_stats = observed_stats, iterations = 1000, mutation_rate = 1e-8, execution = "future_lapply")},
  times = 1
)
saveRDS(bench, "/tmp/bench.rds")
```

```{r}
data <- simulate_abc(model, priors, summary_funs = functions, observed_stats = observed_stats, iterations = 5, mutation_rate = 1e-8, execution = "mclapply")
```

```{r}
data <- simulate_abc(model, priors, summary_funs = functions, observed_stats = observed_stats, iterations = 123, mutation_rate = 1e-8, execution = "future_lapply")
```
...
```{r, echo=FALSE}
saveRDS(data, "inst/extdata/vignette_01_gene_flow.rds")
