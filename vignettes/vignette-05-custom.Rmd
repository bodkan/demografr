...
abc_path <- here::here("inst/examples/custom_abc.rds")
...
```{r}
# pkg_path <- system.file("examples/", package = "demografr")
pkg_path <- "/project/inst/examples"
Sys.setenv(PATH = paste0(dirname(reticulate::py_config()$python),
                         ":",
                         Sys.getenv("PATH"),
                         ":",
                         pkg_path))
...
compute_diversity <- function(vcf) {
  # get the path to an external command-line example program
  program_path <- system.file("examples/custom_diversity", package = "demografr")

  # where to save diversity computed by the program
  tsv <- tempfile()

  # compose the shell command and run it, saving the diversity to a file
  cmd <- paste(program_path, "--vcf", vcf, "--tsv", tsv)
  system(cmd)
