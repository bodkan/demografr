...
```{r, echo=FALSE, eval=!on_windows && !file.exists(path_observed)}
...
```{r, eval=!on_windows && !file.exists(abc_path1) && Sys.getenv("RUNNER_OS") == ""}
library(future)
...
```{r, eval=!on_windows && !file.exists(abc_path2) && Sys.getenv("RUNNER_OS") == ""}
library(future)
plan(multisession, workers = availableCores()) # parallelize across all CPUs
...
```{r, eval=!on_windows && !file.exists(abc_path3) && Sys.getenv("RUNNER_OS") == ""}
library(future)
...
```{r, eval=!on_windows && !file.exists(abc_path4) && Sys.getenv("RUNNER_OS") == ""}
library(future)
plan(multisession, workers = availableCores())
