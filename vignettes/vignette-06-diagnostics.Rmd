...
arrows(1.5, 1.5, 1.5, 3.5, col="blue")
...
cv_result <- run_cv(models = models, nval = 100, tol = 0.01, method = "neuralnet")
```
...
The confusion matrices and the visualization all suggest that ABC can distinguish between the three models very well. For instance, _modelX_ has been classified correctly in:

```{r, echo=FALSE}
cv_result$estim[[1]] %>% { .[names(.) == "modelX"] } %>% { names(.) == . } %>% sum
```

simulations out of the total of 100 cross-validation simulations, with an overall misclassification rate for all models of only:

```{r, echo=FALSE}
cat(sprintf("%.1f%%", cv_result$estim[[1]] %>%  { 1 - mean(names(.) == .) } * 100))
```

## Model selection

Armed with confidence in the ability of ABC to correctly identify the correct model based on simulated data, we can proceed to selection of _the best model for our empirical data set_.
...
models <- list(modelX = abcX, modelY = abcY, modelZ = abcZ)
  ...
  observed_stats <- attr(abcX, "components")$observed
    # and to the same for the observed statistics as well
  observed_stats <- lapply(names(observed_stats), function(stat) {
...
modsel <- postpr(observed_stats, model_names, model_stats, tol = 0.05, method = "neuralnet")
...
sim_stats <- attr(abcX, "components")$simulated
...
#observed_stats <- attr(abcX, "observed")
sim_stats <- attr(abcY, "components")$simulated
...
predictions <- predict(abcX, 1000, posterior = "adj")
predictions
```
