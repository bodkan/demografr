...
When the posterior probabilities are computed using a more elaborate neural network method (again, see more details in the _abc_ [vignette](https://cran.r-project.org/package=abc/vignettes/abcvignette.pdf)), we find that the probability of "modelX" is even more overwhelming. In fact, the analysis shows that it has a nearly 100% probabiliy of being the correct model to explain our data.

**But how accurate are these conclusions?** Well, if we take a peek at the _slendr_ model which was [internally used](https://github.com/bodkan/demografr/blob/main/vignettes/vignette-01-basics.Rmd#L36-L47) to generate the "observed" summary statistics, we see that the data was indeed simulated by code nearly identical to the one shown above as `modelX`.
...
```{r}
gfit <- goodness_of_fit(abcX, replicates = 100)

summary(gfit)

plot(gfit, main = "Histogram for modelX under H0")
```

```{r}
gfit <- goodness_of_fit(abcY, replicates = 100)

summary(gfit)

plot(gfit, main = "Histogram for modelY under H0")
```

```{r}
gfit <- goodness_of_fit(abcZ, replicates = 100)

summary(gfit)

plot(gfit, main = "Histogram for modelZ under H0")
```
...
colnames(observed_stats) <- colnames(model_stats)
