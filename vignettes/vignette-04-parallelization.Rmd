...
functions <- list(
  diversity = function(ts) {
    ts_diversity(ts, sample_sets = extract_names(ts, split = "pop"))
  }
)
```
...
```{r}
data <- simulate_grid(model, grid, functions, replicates = 15, sequence_length = 10000, recombination_rate = 1e-8)
...
```{r}
library(future)
plan(multicore, workers = 15) # use all 10 available CPUs
```
...
```{r}
data <- simulate_grid(model, grid, functions, replicates = 15, sequence_length = 100000, recombination_rate = 1e-8)
...
# on machine #3
simulate_grid(model, grid, functions, replicates = 5, sequence_length = 100000, recombination_rate = 1e-8, file = run3_file)
```

Then, once we collected the individual `.rds` files on a single computer from the remote machines (perhaps using something like the `scp` unix command), we can use another _demografr_ function `combine_data()` to merge all individual runs into a single result:

```{r}
data <- combine_data(run1_file, run2_file, run3_file)

data
```

For each combination of parameters we ran four replicates across "five different machines", totalling `r nrow(grid)` x 2 x 5 = `r nrow(grid) * 2 * 5` so we should get the same number of rows in the merged data frame:
...
```{r}
combined1 <- combine_data(run1_file, run2_file, run3_file)
combined2 <- combine_data(list(run1_file, run2_file, run3_file))
```
...
combined3 <- combine_data(run1, run2, run3)
combined4 <- combine_data(list(run1, run2, run3))
```

```{r, eval=FALSE}
run1 <- simulate_grid(model, grid, functions, replicates = 5, sequence_length = 100000, recombination_rate = 0)
run2 <- simulate_grid(model, grid, functions, replicates = 5, sequence_length = 100000, recombination_rate = 0)
run3 <- simulate_grid(model, grid, functions, replicates = 5, sequence_length = 100000, recombination_rate = 0)
