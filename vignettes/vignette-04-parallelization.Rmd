...
The simplest way to parallelize _demografr_ analyses is via the R package [_future_](https://cran.r-project.org/package=future).
...
In order to use _future_ effectively, you should study its documentation and tutorials (a good start would be the vignette [here](https://cran.r-project.org/package=future)), but here is a brief introduction focused on parallelizing a toy _demografr_ analysis set up above:
...
```{r}
library(future)
plan(multicore, workers = 10) # use 10 CPUs
```
...
If you read the documentation of the _future_ package (which you really [should](https://cran.r-project.org/package=future)), you will discover many more options for parallelization giving you near limitless options for scheduling parallel simulations.
...
For each combination of parameters we ran five replicates across "two different machines", totalling `r nrow(grid)` x 5 x 2 = `r nrow(grid) * 5 * 2` simulations, so we should get the same number of rows in the merged data frame:
