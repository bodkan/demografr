  ...
  data <- readRDS(file = here::here("inst/paper/abc_data.rds"))
  tstart <- Sys.time()

  data <- simulate_abc(
    model, priors, functions, observed,
    iterations = 10000,
    sequence_length = 20e6, recombination_rate = 1e-8, mutation_rate = 1e-8
  )

  saveRDS(data, file = here::here("inst/paper/abc_data.rds"))
  tend <- Sys.time()
  tend - tstart # Time difference of 47.32914 mins
  ...
  predictions <- readRDS(file = here::here("inst/paper/abc_predictions.rds"))
  tstart <- Sys.time()

  predictions <- predict(abc, samples = 1000)

  saveRDS(data, file = here::here("inst/paper/abc_predictions.rds"))
  tend <- Sys.time()
  tend - tstart # Time difference of 3.778034 mins
